...
class TimetableData {
  ...
  List<List<String>> locations = List.generate(
    5,
    (_) => List.generate(7, (_) => ''),
  );
  List<List<String>> teachers = List.generate(
    5,
    (_) => List.generate(7, (_) => ''),
  );
  String name = '我的課表';
  ...
  Future<void> load() async {
    ...
    if (locData != null) {
      ...
      locations = List<List<String>>.from(
        decoded.map((row) => List<String>.from(row)),
      );
    }
    ...
    if (teacherData != null) {
      ...
      teachers = List<List<String>>.from(
        decoded.map((row) => List<String>.from(row)),
      );
    }
...
class _TimetableImporterState extends State<TimetableImporter> {
  ...
  void _showSetPeriodsDialog() async {
    final controller = TextEditingController(
      text: timetable.periods.toString(),
    );
    await showDialog(
      context: context,
      builder: (context) => AlertDialog(
        ...
        title: Text(
          AppLocalizations.of(context).t('set_periods_title'),
        ...
        actions: [
          IconButton(
            ...
            tooltip: isEditing
                ...
                : AppLocalizations.of(context).t('edit_periods'),
            onPressed: () {
              setState(() {
                          ...
                          children: [
                            SizedBox(
                              ...
                              child: Text(
                                AppLocalizations.of(
                                  context,
                                ).tWithNumber('period_format', period + 1),
                                style: TextStyle(
                                  fontWeight: FontWeight.bold,
                                      ...
                                      onLongPress: () async {
                                        final subjectController =
                                            TextEditingController(
                                              text:
                                                  timetable.table[day][period],
                                            );
                                        final locationController =
                                            TextEditingController(
                                              text: timetable
                                                  .locations[day][period],
                                            );
                        ...
                        builder: (context) {
                          ...
                          String getSubject(int period) {
                            ...
                            return subject.isEmpty
                                ? AppLocalizations.of(context).t('no_scheduled')
                                : subject;
                          }
