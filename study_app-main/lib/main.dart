          ...
          builder: (context, seedColor, _) {
            return ValueListenableBuilder<ThemeMode>(
              ...
              builder: (context, mode, _) =>
                  MyApp(themeMode: mode, seedColor: seedColor, locale: locale),
            );
          },
        );
      },
...
class _MainPageState extends State<MainPage> {
  ...
  late final GlobalKey<_HomePageState> _homePageKey = GlobalKey();

  @override
  void initState() {
    super.initState();
    _pages = [
      HomePage(key: _homePageKey, onQuickNav: _onItemTapped),
      TimetableImporter(),
      LabelEngine(),
      HomeworkPage(),
      const SettingsPage(),
  ...
  void _onItemTapped(int index) {
    setState(() {
      ...
      // 當切換回首頁時，重新加載功課列表
      if (index == 0) {
        _homePageKey.currentState?._loadUpcomingHomeworks();
      }
    });
  }
...
class _HomePageState extends State<HomePage> with WidgetsBindingObserver {
  ...
  String getNextClass() {
    try {
      ...
      if (nextPeriod < 1 || nextPeriod > timetable.periods) {
